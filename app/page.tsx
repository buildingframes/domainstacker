...
function fallbackIdeas(input: string): Idea[] {
  ...
  return names.map((n, i) => ({
    name: n,
    domain: `${n.toLowerCase()}.com`,
    angle: ['Clear and practical', 'Founder-friendly', 'Modern SaaS feel', 'Strong utility vibe', 'Simple and memorable', 'Action-oriented'][i],
    score: 92 - i * 3,
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[#f8fbfc]">
      ...
      <div className="grid gap-8 md:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="grid gap-4">
          {ideas.map((x) => {
            ...
            return <div key={x.name} className="rounded-3xl border bg-white p-5 shadow-sm">
    ...
    <section id="stack" className="mx-auto max-w-6xl px-6 py-16"><div className="mb-8"><h2 className="text-3xl font-black">Recommended launch stack</h2><p className="mt-2 text-slate-600">Replace placeholder URLs with your approved affiliate links.</p></div><div className="grid gap-4 md:grid-cols-4">{stacks.map((s) => <a key={s.name} href={s.url} target="_blank" className="rounded-3xl border bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><Star className="text-cyanframe" /><h3 className="mt-4 font-bold">{s.name}</h3><p className="mt-1 text-sm text-slate-500">{s.use}</p></a>)}</div></section>
